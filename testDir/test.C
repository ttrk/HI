...
void plotSame()
{
	  ...
	  TList* histos_pt40=getListOfALLHistograms(new TFile(Form("%s/photonCuts_19032015_pt40_run0.root",dirName),"READ"));
	  TList* histos_pt60=getListOfALLHistograms(new TFile(Form("%s/photonCuts_19032015_pt60_run0.root",dirName),"READ"));
	  TList* histos_pt80=getListOfALLHistograms(new TFile(Form("%s/photonCuts_19032015_pt80_run0.root",dirName),"READ"));
...
//			  c1->SetLogy();		// some curves may not appear in linear scale
			  ...
			  c1->SaveAs(Form("%s/drawsameAll/%s.gif",dirName ,c1->GetName()));
	  }
	  c1->Close();
}
